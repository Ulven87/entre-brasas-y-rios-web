      ...
      const observerCallback = (entries, observer) => {
          ...
          entries.forEach(entry => {
              if (entry.isIntersecting) {
                  ...
                  // Usa navLinks definido arriba
                  const correspondingLink = document.querySelector(`header nav > ul > li > a[href="#${id}"]`); // Selector corregido

                  // Usa currentlyActiveLink definido y actualizado arriba (CORREGIDO)
                  if (correspondingLink && correspondingLink !== currentlyActiveLink) {
                      // console.log(`Observer: Intersecting #${id} (v18).`); // Log opcional
